...
class SimplifyRm1TestCase(unittest.TestCase):
    ...
    def testSimplifyRm1_with_bridge_elimination(self):
        knot = create_knot_from_pd_code([[1,2,2,3],[3,6,4,7],[7,4,8,5],[8,6,1,5]], bridges = {0:[2,3]})
        knot.simplify_rm1([0])
        answer = create_knot_from_pd_code([[1,4,2,5],[5,2,6,3],[6,4,1,3]])
        self.assertEqual(knot, answer)
        self.assertEqual(knot.bridges, {})

    def testSimplifyRm1_with_bridge_merger(self):
        knot = create_knot_from_pd_code([[1,2,2,3],[3,4,4,5],[6,5,1,6]], bridges = {0:[2,3], 1:[6,5]})
        knot.simplify_rm1([1])
        answer = create_knot_from_pd_code([[1,2,2,3],[4,3,1,4]], bridges = {0:[2,4]})
        self.assertEqual(knot, answer)
...
class SimplifyRm2TestCase(unittest.TestCase):
    ...
    def testSimplifyRm2_with_bridge_elimination(self):
        knot = create_knot_from_pd_code([[1,8,2,1],[4,6,5,5],[6,3,7,4],[7,3,8,2]], bridges = {0:[2,3]})
